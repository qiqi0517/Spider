...
| 部分 | 主要技术 | 用途 |
| --- | --- | --- |
| 运行环境 | Python 3.10、Conda | 隔离项目依赖 |
| 动态爬取 | Playwright | 打开 QQ 音乐动态页面、等待元素和滚动列表 |
| 图片请求 | Requests、Pillow | 复用会话下载并校验图片 |
| 数据保存 | JSON、原子文件替换 | 保存原始结构化数据和检查点 |
| Web 后端 | Django 5.2.15 | URL 路由、模型、视图、表单处理和模板渲染 |
| 数据库 | SQLite | 保存歌曲、歌手、关系与评论 |
| 前端 | Django Template、Bootstrap 5 | 页面布局、卡片、表单和响应式样式 |
| 搜索 | 倒排索引、规则排序、TF-IDF | 候选筛选和相关度排序 |
| 数据分析 | Pandas、jieba、WordCloud、scikit-learn、Sentence Transformers | 统计、分词、词云、聚类和降维 |
| 版本管理 | Git | 按爬虫、Web 和分析阶段保存提交记录 |
...
由用户正常完成登录，再将 Cookie 和浏览器存储状态保存到仅供本机使用的配置文件。爬虫入口在启动时检查这些文件是否存在，缺少时给出明确提示。
...
6. 数据页面完成后，使用同一个 Requests Session 下载歌手图片和歌曲图片。

页面打开失败时最多重试 3 次，并逐步增加等待时间；外层采集任务也设置了有限次数的重试和最大等待时间。页面访问后会随机等待，请求间隔由配置统一控制，图片下载同样保留 1～2 秒间隔。项目没有抓取音频或视频文件。
...
- `Singer` 保存姓名、原网页 URL、本地图片、图片 URL 和简介；
- `Song` 保存歌曲名、原网页 URL、本地图片、图片 URL、歌词，并通过多对多字段关联歌手；
- `Comment` 通过外键关联歌曲，保存评论正文和时间。
...
- 较长歌词默认只展示前 20 行，用户可以点击 `展开歌词` 查看完整内容。展开后，按钮自动变为 `折叠歌词` ，再次点击即可收起。
- 歌曲下方展示评论列表
...
为了在 3835 首歌曲的歌词中进行子串搜索，同时将响应时间控制在 1 秒以内，项目预先用管理命令 `build_inverted_index` 构建倒排索引，并计算TF-IDF。搜索时执行以下流程：

1. 管理命令分别为歌曲和歌手生成一元、二元字符倒排索引
...
注：索引只在第一次进入相应搜索视图时加载，后续请求复用内存缓存，普通列表页不会承担索引加载成本。
