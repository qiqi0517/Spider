...
## 结论二：歌词和热门评论的用词重点不同：歌词偏向作品表达，评论偏向评价与回应
...
- 清洗歌词并分词
- 过滤清洗后长度不足 30 字符的歌词，再将长歌词按照原有行顺序切分为最长 300 字符的文本块
- 使用预训练 embedding 模型 `BAAI/bge-small-zh-v1.5` 将每个文本块转化为 512 维向量
...
t-SNE 可视化图表中，按照 cluster 进行分类染色，得到如下结果：

![歌词主题聚类 t-SNE 可视化](../analysis/output/lyrics_tsne/lyrics_theme_tsne.png)

### 各聚类主题与典型歌曲

下表展示全部 32 个 cluster 的信息摘要。其中典型歌曲选自距离 K-Means 聚类中心较近的歌曲。主题名称和分析是根据关键词、中心歌曲和歌词内容人工归纳的，重点比较不同 cluster 的区别。
...
数据分析结果中，3789 首歌词并没有完全混在一起，而是分出了不少可以解释的簇。特别是几个相近的爱情主题还能继续分成告白、失恋、拉扯、爱而不得和犹豫等不同情况，说明歌曲之间的歌词语义差异并不只是在歌曲主题（例如爱情、自然、社会、人生等）这一层，歌词里的情境、情绪和常用意象也会形成更细的区别。梦想类歌曲也有类似现象：Cluster 0 更强调坚持和前进，Cluster 20 更偏少年时期的快乐和梦想，Cluster 11 则更多写现实压力下的漂泊和坚持。
...
3. 歌词语义可以形成多个具有明确特征的聚类。根据歌词语义，可识别出追梦励志、古风相思、失恋遗忘、故乡旅途、雨夜思念、海洋离别、说唱态度和多语种流行等主题或表达类型。
